...
function getHeadersWithAuth(appConfig2, { refreshToken }) {
  const headers = getHeaders(appConfig2);
 ...
 * limitations under the License.
 */
async function createInstallationRequest({ appConfig: appConfig2, heartbeatServiceProvider }, { fid }) {
  const endpoint = getInstallationsEndpoint(appConfig2);
  const headers = getHeaders(appConfig2);
 ...
 * limitations under the License.
 */
function getKey(appConfig2) {
  return `${appConfig2.appName}!${appConfig2.appId}`;
}
/**
 * @license
 * Copyright 2019 Google LLC
...
function fidChanged(appConfig2, fid) {
  const key = getKey(appConfig2);
...
async function set(appConfig2, value) {
  const key = getKey(appConfig2);
  ...
  if (!oldValue || oldValue.fid !== value.fid) {
    fidChanged(appConfig2, value.fid);
  }
  return value;
}
async function remove(appConfig2) {
  const key = getKey(appConfig2);
  const db = await getDbPromise();
  const tx = db.transaction(OBJECT_STORE_NAME, "readwrite");
...
async function update(appConfig2, updateFn) {
  const key = getKey(appConfig2);
  const db = await getDbPromise();
  const tx = db.transaction(OBJECT_STORE_NAME, "readwrite");
  ...
  if (newValue && (!oldValue || oldValue.fid !== newValue.fid)) {
    fidChanged(appConfig2, newValue.fid);
  }
  return newValue;
}
/**
...
function updateInstallationRequest(appConfig2) {
  return update(appConfig2, (oldEntry) => {
    if (!oldEntry) {
 ...
 * limitations under the License.
 */
async function generateAuthTokenRequest({ appConfig: appConfig2, heartbeatServiceProvider }, installationEntry) {
  const endpoint = getGenerateAuthTokenEndpoint(appConfig2, installationEntry);
  const headers = getHeadersWithAuth(appConfig2, installationEntry);
  const heartbeatService = heartbeatServiceProvider.getImmediate({
    optional: true
  });
  if (heartbeatService) {
...
function getGenerateAuthTokenEndpoint(appConfig2, { fid }) {
  return `${getInstallationsEndpoint(appConfig2)}/${fid}/authTokens:generate`;
}
/**
 * @license
 * Copyright 2019 Google LLC
...
function updateAuthTokenRequest(appConfig2) {
  return update(appConfig2, (oldEntry) => {
    if (!isEntryRegistered(oldEntry)) {
      throw ERROR_FACTORY.create(
        "not-registered"
...
var publicFactory = (container) => {
  ...
  const appConfig2 = extractAppConfig(app);
...
async function fetchDynamicConfigWithRetry(app, retryData = defaultRetryData, timeoutMillis) {
  ...
  const signal2 = new AnalyticsAbortSignal();
  setTimeout(async () => {
    signal2.abort();
  }, timeoutMillis !== void 0 ? timeoutMillis : FETCH_TIMEOUT_MILLIS);
  return attemptFetchDynamicConfigWithRetry({ appId, apiKey, measurementId }, throttleMetadata, signal2, retryData);
}
async function attemptFetchDynamicConfigWithRetry(appFields, { throttleEndTimeMillis, backoffCount }, signal2, retryData = defaultRetryData) {
  const { appId, measurementId } = appFields;
  try {
    await setAbortableTimeout(signal2, throttleEndTimeMillis);
  } catch (e) {
    if (measurementId) {
  ...
  try {
    ...
    return attemptFetchDynamicConfigWithRetry(appFields, throttleMetadata, signal2, retryData);
  }
}
function setAbortableTimeout(signal2, throttleEndTimeMillis) {
...
var UserTrackingService = class _UserTrackingService {
  ...
  ngOnDestroy() {
    this.disposables.forEach((it2) => it2.unsubscribe());
  }
...
var \u0275screenViewEvent = (router, title, componentFactoryResolver) => {
  ...
  return activationEndEvents.pipe(switchMap((activationEnd) => {
    ...
    const actualSnapshot = router.routerState.root.children.map((it2) => it2).find((it2) => it2.outlet === activationEnd.snapshot.outlet);
    if (!actualSnapshot) {
      return of(null);
    }
    ...
    const screenName = actualDeep.pathFromRoot.map((s) => s.routeConfig?.path).filter((it2) => it2).join("/") || "/";
    const params = {
    ...
    return of(null);
  }), filter((it2) => !!it2), map((params) => __spreadValues({
    [FIREBASE_SCREEN_CLASS_KEY]: params[SCREEN_CLASS_KEY],
    [FIREBASE_SCREEN_INSTANCE_ID_KEY]: getScreenInstanceID(params)
  }, params)), groupBy((it2) => it2[OUTLET_KEY]), mergeMap((it2) => it2.pipe(distinctUntilChanged((a, b2) => JSON.stringify(a) === JSON.stringify(b2)), startWith(void 0), pairwise(), map(([prior, current]) => prior ? __spreadValues({
...
var adminGuard = () => {
  ...
  return userService.ensureProfile().pipe(take(1), map((profile) => profile.role === "super_admin" ? true : router.createUrlTree(["/app/home"])), catchError(() => of(router.createUrlTree(["/app/home"]))));
};
...
var authGuard = () => {
  ...
  const router = inject(Router);
  return resolveFirebaseUser(auth).pipe(map((user) => user ? true : router.createUrlTree(["/login"])));
};
var emailVerifiedGuard = () => {
  const auth = inject(Auth);
  const router = inject(Router);
  return resolveFirebaseUser(auth).pipe(switchMap((user) => {
    if (!user) {
      return of(router.createUrlTree(["/login"]));
    }
    return from(user.reload()).pipe(map(() => {
...
function redirectOnProfileError(err, router, fallback) {
...
var onboardingGuard = () => {
  ...
  const router = inject(Router);
  return userSvc.ensureProfile().pipe(take(1), map((profile) => profile.onboarding_completed ? true : router.createUrlTree(["/app/onboarding"])), catchError((err) => redirectOnProfileError(err, router, "/app/onboarding")));
};
var onboardingPageGuard = () => {
  const userSvc = inject(UserService);
  const router = inject(Router);
  return userSvc.getProfile().pipe(take(1), map((profile) => profile.onboarding_completed ? router.createUrlTree(["/app/home"]) : true), catchError((err) => redirectOnProfileError(err, router, "/app/onboarding")));
};
var dataConsentGuard = () => {
  const userSvc = inject(UserService);
  const auth = inject(AuthService);
  const router = inject(Router);
  return userSvc.ensureProfile().pipe(take(1), switchMap((profile) => {
    if (profile.data_consent_accepted !== false) {
      return of(true);
    }
    return auth.logout().pipe(map(() => router.createUrlTree(["/login"], {
...
// src/app/core/resolvers/onboarding-profile.resolver.ts
var onboardingProfileResolver = () => inject(UserService).ensureProfile().pipe(take(1));

// src/app/app.routes.ts
var routes = [
  {
    path: "",
    loadComponent: () => import("./chunk-T643RSAQ.js").then((m) => m.LandingComponent),
    data: { title: "landing" }
  },
  {
    path: "login",
  ...
  },
  {
    ...
    loadComponent: () => import("./chunk-C3OGCODF.js").then((m) => m.LegalDocumentComponent),
    data: { doc: "data-processing", title: "legalDataProcessing" }
  },
  {
    path: "legal/cookies",
    loadComponent: () => import("./chunk-C3OGCODF.js").then((m) => m.LegalDocumentComponent),
    data: { doc: "cookies", title: "legalCookies" }
  },
  {
    path: "app",
...
var authInterceptor = (req, next) => {
  ...
  return from(authService.getIdToken()).pipe(switchMap((token) => {
    if (token) {
...
// src/app/core/interceptors/email-verification.interceptor.ts
var emailVerificationInterceptor = (req, next) => {
  const router = inject(Router);
...
function NavbarComponent_Conditional_38_Template(rf, ctx) {
  if (rf & 1) {
    ...
    \u0275\u0275element(2, "path", 44)(3, "path", 45)(4, "rect", 46)(5, "path", 47);
    \u0275\u0275elementEnd();
    \u0275\u0275namespaceHTML();
  ...
  if (rf & 2) {
    ...
    \u0275\u0275advance(7);
    \u0275\u0275textInterpolate(ctx_r0.i18n.nav().admin);
  }
}
function NavbarComponent_Conditional_78_Template(rf, ctx) {
  if (rf & 1) {
  ...
  if (rf & 2) {
    const ctx_r0 = \u0275\u0275nextContext();
    \u0275\u0275advance(6);
    \u0275\u0275textInterpolate1(" ", ctx_r0.i18n.nav().admin, " ");
  }
}
var SIDEBAR_COLLAPSE_BTN_MAX = 890;
var NavbarComponent = class _NavbarComponent {
  ...
  constructor() {
    effect(() => {
      document.documentElement.style.setProperty("--sidebar-w", this.collapsed() ? "64px" : "220px");
    });
  ...
  static \u0275cmp = /* @__PURE__ */ \u0275\u0275defineComponent({ type: _NavbarComponent, selectors: [["app-navbar"]], decls: 84, vars: 21, consts: [[1, "sidebar"], [1, "sidebar-head"], [1, "sidebar-logo"], ["type", "button", 1, "sidebar-logo-toggle", 3, "click"], ["aria-hidden", "true", 1, "sidebar-logo-toggle__letter"], ["width", "20", "height", "20", "viewBox", "0 0 24 24", "fill", "none", "stroke", "currentColor", "stroke-width", "2", "stroke-linecap", "round", "aria-hidden", "true", 1, "sidebar-logo-toggle__close"], [1, "nav-links"], ["routerLink", "/app/home", "routerLinkActive", "active", 1, "nav-link"], ["width", "20", "height", "20", "viewBox", "0 0 24 24", "fill", "none", "stroke", "currentColor", "stroke-width", "2"], ["x", "3", "y", "3", "width", "7", "height", "7"], ["x", "14", "y", "3", "width", "7", "height", "7"], ["x", "3", "y", "14", "width", "7", "height", "7"], ["x", "14", "y", "14", "width", "7", "height", "7"], ["routerLink", "/app/students", "routerLinkActive", "active", 1, "nav-link"], ["d", "M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"], ["cx", "9", "cy", "7", "r", "4"], ["d", "M23 21v-2a4 4 0 0 0-3-3.87"], ["d", "M16 3.13a4 4 0 0 1 0 7.75"], ["routerLink", "/app/calendar", "routerLinkActive", "active", 1, "nav-link"], ["x", "3", "y", "4", "width", "18", "height", "18", "rx", "2"], ["x1", "16", "y1", "2", "x2", "16", "y2", "6"], ["x1", "8", "y1", "2", "x2", "8", "y2", "6"], ["x1", "3", "y1", "10", "x2", "21", "y2", "10"], ["routerLink", "/app/finance", "routerLinkActive", "active", 1, "nav-link"], ["x1", "12", "y1", "1", "x2", "12", "y2", "23"], ["d", "M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6"], ["routerLink", "/app/admin", "routerLinkActive", "active", 1, "nav-link"], [1, "sidebar-footer"], ["routerLink", "/app/account", "routerLinkActive", "active", 1, "sidebar-btn", "sidebar-btn--account"], ["d", "M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"], ["cx", "12", "cy", "7", "r", "4"], ["type", "button", 1, "sidebar-btn", "logout", 3, "click"], ["d", "M9 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h4"], ["points", "16 17 21 12 16 7"], ["x1", "21", "y1", "12", "x2", "9", "y2", "12"], [1, "bottom-nav"], ["routerLink", "/app/home", "routerLinkActive", "active", 1, "bottom-link"], ["viewBox", "0 0 24 24", "fill", "none", "stroke", "currentColor", "stroke-width", "2"], ["routerLink", "/app/students", "routerLinkActive", "active", 1, "bottom-link"], ["routerLink", "/app/calendar", "routerLinkActive", "active", 1, "bottom-link"], ["routerLink", "/app/finance", "routerLinkActive", "active", 1, "bottom-link"], ["routerLink", "/app/admin", "routerLinkActive", "active", 1, "bottom-link"], ["routerLink", "/app/account", "routerLinkActive", "active", 1, "bottom-link"], ["d", "M18 6 6 18M6 6l12 12"], ["d", "M12 15v2"], ["d", "M12 3v2"], ["x", "4", "y", "8", "width", "16", "height", "12", "rx", "2"], ["d", "M8 8V6a4 4 0 0 1 8 0v2"]], template: function NavbarComponent_Template(rf, ctx) {
    if (rf & 1) {
      ...
      \u0275\u0275conditionalCreate(5, NavbarComponent_Conditional_5_Template, 2, 0, "span", 4)(6, NavbarComponent_Conditional_6_Template, 2, 0, ":svg:svg", 5);
      \u0275\u0275elementEnd()();
      \u0275\u0275elementStart(7, "nav", 6)(8, "a", 7);
      \u0275\u0275namespaceSVG();
      \u0275\u0275elementStart(9, "svg", 8);
      \u0275\u0275element(10, "rect", 9)(11, "rect", 10)(12, "rect", 11)(13, "rect", 12);
      \u0275\u0275elementEnd();
      \u0275\u0275namespaceHTML();
      ...
      \u0275\u0275element(18, "path", 14)(19, "circle", 15)(20, "path", 16)(21, "path", 17);
      \u0275\u0275elementEnd();
      \u0275\u0275namespaceHTML();
      ...
      \u0275\u0275element(26, "rect", 19)(27, "line", 20)(28, "line", 21)(29, "line", 22);
      \u0275\u0275elementEnd();
      \u0275\u0275namespaceHTML();
      ...
      \u0275\u0275namespaceSVG();
      \u0275\u0275elementStart(33, "svg", 8);
      \u0275\u0275element(34, "line", 24)(35, "path", 25);
      \u0275\u0275elementEnd();
      \u0275\u0275namespaceHTML();
      \u0275\u0275elementStart(36, "span");
      \u0275\u0275text(37);
      \u0275\u0275elementEnd()();
      \u0275\u0275conditionalCreate(38, NavbarComponent_Conditional_38_Template, 8, 1, "a", 26);
      \u0275\u0275elementEnd();
      \u0275\u0275elementStart(39, "div", 27)(40, "a", 28);
      \u0275\u0275namespaceSVG();
      \u0275\u0275elementStart(41, "svg", 8);
      \u0275\u0275element(42, "path", 29)(43, "circle", 30);
      \u0275\u0275elementEnd();
      \u0275\u0275namespaceHTML();
      ...
      \u0275\u0275listener("click", function NavbarComponent_Template_button_click_46_listener() {
        return ctx.logout();
      });
      \u0275\u0275namespaceSVG();
      \u0275\u0275elementStart(47, "svg", 8);
      \u0275\u0275element(48, "path", 32)(49, "polyline", 33)(50, "line", 34);
      \u0275\u0275elementEnd();
      \u0275\u0275namespaceHTML();
      \u0275\u0275elementStart(51, "span");
      \u0275\u0275text(52);
      \u0275\u0275elementEnd()()()();
      \u0275\u0275elementStart(53, "nav", 35)(54, "a", 36);
      \u0275\u0275namespaceSVG();
      \u0275\u0275elementStart(55, "svg", 37);
      \u0275\u0275element(56, "rect", 9)(57, "rect", 10)(58, "rect", 11)(59, "rect", 12);
      \u0275\u0275elementEnd();
      \u0275\u0275text(60);
      \u0275\u0275elementEnd();
      \u0275\u0275namespaceHTML();
      \u0275\u0275elementStart(61, "a", 38);
      \u0275\u0275namespaceSVG();
      \u0275\u0275elementStart(62, "svg", 37);
      \u0275\u0275element(63, "path", 14)(64, "circle", 15);
      \u0275\u0275elementEnd();
      \u0275\u0275text(65);
      \u0275\u0275elementEnd();
      \u0275\u0275namespaceHTML();
      ...
      \u0275\u0275element(68, "rect", 19)(69, "line", 20)(70, "line", 21)(71, "line", 22);
      \u0275\u0275elementEnd();
      \u0275\u0275text(72);
      \u0275\u0275elementEnd();
      \u0275\u0275namespaceHTML();
      ...
      \u0275\u0275namespaceSVG();
      \u0275\u0275elementStart(74, "svg", 37);
      \u0275\u0275element(75, "line", 24)(76, "path", 25);
      \u0275\u0275elementEnd();
      \u0275\u0275text(77);
      \u0275\u0275elementEnd();
      \u0275\u0275conditionalCreate(78, NavbarComponent_Conditional_78_Template, 7, 1, "a", 41);
      \u0275\u0275namespaceHTML();
      \u0275\u0275elementStart(79, "a", 42);
      \u0275\u0275namespaceSVG();
      \u0275\u0275elementStart(80, "svg", 37);
      \u0275\u0275element(81, "path", 29)(82, "circle", 30);
      \u0275\u0275elementEnd();
      \u0275\u0275text(83);
      \u0275\u0275elementEnd()();
    }
    if (rf & 2) {
      ...
      \u0275\u0275advance(4);
      \u0275\u0275classProp("sidebar-logo-toggle--collapsed", ctx.collapsed());
      \u0275\u0275attribute("aria-label", ctx.collapsed() ? ctx.i18n.nav().sidebarExpand : ctx.i18n.nav().sidebarCollapse)("aria-expanded", !ctx.collapsed())("title", ctx.collapsed() ? ctx.i18n.nav().sidebarExpand : ctx.i18n.nav().sidebarCollapse);
      \u0275\u0275advance();
      \u0275\u0275conditional(ctx.collapsed() ? 5 : 6);
      \u0275\u0275advance(10);
      \u0275\u0275textInterpolate(ctx.i18n.nav().home);
      \u0275\u0275advance(8);
      \u0275\u0275textInterpolate(ctx.i18n.nav().students);
      \u0275\u0275advance(8);
      \u0275\u0275textInterpolate(ctx.i18n.nav().calendar);
      \u0275\u0275advance(6);
      \u0275\u0275textInterpolate(ctx.i18n.nav().finance);
      \u0275\u0275advance();
      \u0275\u0275conditional(ctx.isSuperAdmin() ? 38 : -1);
      \u0275\u0275advance(7);
      \u0275\u0275textInterpolate(ctx.i18n.nav().account);
      \u0275\u0275advance(7);
      \u0275\u0275textInterpolate(ctx.i18n.nav().logout);
      \u0275\u0275advance(8);
      \u0275\u0275textInterpolate1(" ", ctx.i18n.nav().home, " ");
      \u0275\u0275advance(5);
      \u0275\u0275textInterpolate1(" ", ctx.i18n.nav().students, " ");
      \u0275\u0275advance(7);
      \u0275\u0275textInterpolate1(" ", ctx.i18n.nav().calendar, " ");
      \u0275\u0275advance(5);
      \u0275\u0275textInterpolate1(" ", ctx.i18n.nav().finance, " ");
      \u0275\u0275advance();
      \u0275\u0275conditional(ctx.isSuperAdmin() ? 78 : -1);
      \u0275\u0275advance(5);
      \u0275\u0275textInterpolate1(" ", ctx.i18n.nav().account, " ");
    }
  }, dependencies: [RouterLink, RouterLinkActive], styles: ["\n[_nghost-%COMP%] {\n  display: contents;\n}\n.sidebar[_ngcontent-%COMP%] {\n  position: fixed;\n  inset: 0 auto 0 0;\n  width: var(--sidebar-w, 220px);\n  height: 100vh;\n  height: 100dvh;\n  height: 100svh;\n  max-height: 100dvh;\n  max-height: 100svh;\n  background: var(--nav-bg);\n  border-right: 1px solid var(--nav-border);\n  display: flex;\n  flex-direction: column;\n  padding: 16px 8px;\n  padding-bottom: max(16px, env(safe-area-inset-bottom, 0px));\n  transition: width 0.25s ease;\n  overflow: hidden;\n  overscroll-behavior: none;\n  touch-action: manipulation;\n  z-index: var(--z-app-navbar, 100);\n  box-sizing: border-box;\n}\n@media (max-width: 768px), (max-height: 440px) {\n  .sidebar[_ngcontent-%COMP%] {\n    display: none;\n  }\n}\n.sidebar-head[_ngcontent-%COMP%] {\n  flex-shrink: 0;\n  padding: 0 4px 12px;\n  min-height: 44px;\n}\n.sidebar-logo[_ngcontent-%COMP%] {\n  font-size: 17px;\n  font-weight: 600;\n  color: var(--nav-accent);\n  padding: 8px 8px 0;\n  white-space: nowrap;\n  overflow: hidden;\n}\n@media (max-width: 890px) {\n  .sidebar-logo[_ngcontent-%COMP%] {\n    display: none;\n  }\n}\n.sidebar-logo-toggle[_ngcontent-%COMP%] {\n  display: none;\n  align-items: center;\n  width: 100%;\n  margin: 0;\n  padding: 10px 12px;\n  border: none;\n  border-radius: 8px;\n  background: transparent;\n  color: var(--nav-text);\n  text-align: left;\n  cursor: pointer;\n  transition: background 0.15s, color 0.15s;\n}\n@media (max-width: 890px) {\n  .sidebar-logo-toggle[_ngcontent-%COMP%] {\n    display: flex;\n  }\n}\n.sidebar-logo-toggle[_ngcontent-%COMP%]:hover {\n  background: var(--nav-hover);\n  color: var(--nav-accent);\n}\n.sidebar-logo-toggle--collapsed[_ngcontent-%COMP%] {\n  justify-content: center;\n  color: var(--nav-accent);\n}\n.sidebar-logo-toggle__letter[_ngcontent-%COMP%] {\n  font-size: 17px;\n  font-weight: 700;\n  line-height: 1;\n}\n.sidebar-logo-toggle__close[_ngcontent-%COMP%] {\n  flex-shrink: 0;\n  opacity: 0.85;\n}\n.nav-links[_ngcontent-%COMP%] {\n  display: flex;\n  flex-direction: column;\n  gap: 4px;\n  flex: 1 1 auto;\n  min-height: 0;\n  overflow: hidden;\n  overscroll-behavior: none;\n  touch-action: none;\n}\n.nav-link[_ngcontent-%COMP%] {\n  display: flex;\n  align-items: center;\n  gap: 12px;\n  padding: 10px 12px;\n  border-radius: 8px;\n  color: var(--nav-text);\n  text-decoration: none;\n  font-size: 14px;\n  white-space: nowrap;\n  touch-action: manipulation;\n  transition: background 0.15s, color 0.15s;\n}\n.nav-link[_ngcontent-%COMP%]   svg[_ngcontent-%COMP%] {\n  flex-shrink: 0;\n}\n.nav-link[_ngcontent-%COMP%]:hover {\n  background: var(--nav-hover);\n}\n.nav-link.active[_ngcontent-%COMP%] {\n  background: var(--nav-active-bg);\n  color: var(--nav-accent);\n  font-weight: 500;\n}\n.nav-link[_ngcontent-%COMP%]   span[_ngcontent-%COMP%] {\n  transition: opacity 0.2s ease;\n}\n.sidebar.collapsed[_ngcontent-%COMP%]   .nav-link[_ngcontent-%COMP%]   span[_ngcontent-%COMP%] {\n  opacity: 0;\n  width: 0;\n  overflow: hidden;\n}\n.sidebar-btn[_ngcontent-%COMP%] {\n  background: none;\n  border: none;\n  cursor: pointer;\n  padding: 10px 12px;\n  border-radius: 8px;\n  color: var(--nav-text);\n  display: flex;\n  align-items: center;\n  gap: 12px;\n  font-size: 14px;\n  white-space: nowrap;\n  width: 100%;\n  touch-action: manipulation;\n  transition: background 0.15s, color 0.15s;\n}\n.sidebar-btn[_ngcontent-%COMP%]   svg[_ngcontent-%COMP%] {\n  flex-shrink: 0;\n}\n.sidebar-btn[_ngcontent-%COMP%]   span[_ngcontent-%COMP%] {\n  transition: opacity 0.2s ease;\n}\n.sidebar.collapsed[_ngcontent-%COMP%]   .sidebar-btn[_ngcontent-%COMP%]   span[_ngcontent-%COMP%] {\n  opacity: 0;\n  width: 0;\n  overflow: hidden;\n}\n.sidebar-btn[_ngcontent-%COMP%]:hover {\n  background: var(--nav-hover);\n}\n@media (max-width: 768px), (max-height: 440px) {\n  .sidebar-btn.logout[_ngcontent-%COMP%] {\n    display: none;\n  }\n}\n.sidebar-btn.logout[_ngcontent-%COMP%]:hover {\n  background: #fce8e6;\n  color: #c5221f;\n}\n.sidebar-footer[_ngcontent-%COMP%] {\n  display: flex;\n  flex-shrink: 0;\n  flex-direction: column;\n  gap: 4px;\n  margin-top: auto;\n  padding-top: 10px;\n  border-top: 1px solid var(--nav-border);\n  box-sizing: border-box;\n}\n.sidebar-btn--account[_ngcontent-%COMP%] {\n  text-decoration: none;\n}\n.sidebar-btn--account.active[_ngcontent-%COMP%] {\n  background: var(--nav-active-bg);\n  color: var(--nav-accent);\n  font-weight: 500;\n}\n.sidebar.collapsed[_ngcontent-%COMP%]   .nav-link[_ngcontent-%COMP%], \n.sidebar.collapsed[_ngcontent-%COMP%]   .sidebar-btn[_ngcontent-%COMP%] {\n  justify-content: center;\n  gap: 0;\n  padding-left: 10px;\n  padding-right: 10px;\n}\n.sidebar.collapsed[_ngcontent-%COMP%]   .sidebar-btn[_ngcontent-%COMP%]   span[_ngcontent-%COMP%] {\n  opacity: 0;\n  width: 0;\n  overflow: hidden;\n}\n.bottom-nav[_ngcontent-%COMP%] {\n  display: none;\n}\n@media (max-width: 768px), (max-height: 440px) {\n  .bottom-nav[_ngcontent-%COMP%] {\n    display: flex;\n    position: fixed;\n    bottom: 0;\n    left: 0;\n    right: 0;\n    height: calc(56px + env(safe-area-inset-bottom, 0px));\n    min-height: 56px;\n    padding-bottom: env(safe-area-inset-bottom, 0px);\n    background: var(--nav-bg);\n    border-top: 1px solid var(--nav-border);\n    justify-content: space-around;\n    align-items: center;\n    overflow: hidden;\n    overscroll-behavior: none;\n    touch-action: none;\n    z-index: var(--z-app-navbar);\n  }\n}\n.bottom-link[_ngcontent-%COMP%] {\n  display: flex;\n  min-width: 0;\n  flex-direction: column;\n  align-items: center;\n  gap: 3px;\n  color: var(--nav-text);\n  text-decoration: none;\n  font-size: 10px;\n  flex: 1;\n  padding: 6px 0;\n  overflow: hidden;\n  text-align: center;\n  line-height: 1.15;\n  transition: color 0.15s;\n}\n.bottom-link[_ngcontent-%COMP%]   svg[_ngcontent-%COMP%] {\n  width: 22px;\n  height: 22px;\n  flex-shrink: 0;\n}\n.bottom-link.active[_ngcontent-%COMP%] {\n  color: var(--nav-accent);\n}\n/*# sourceMappingURL=navbar.component.css.map */"] });
};
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && setClassMetadata(NavbarComponent, [{
    type: Component,
    args: [{ selector: "app-navbar", imports: [RouterLink, RouterLinkActive], template: '<!-- Desktop sidebar -->\n<aside class="sidebar" [class.collapsed]="collapsed()">\n  <div class="sidebar-head">\n    <div class="sidebar-logo">Simple4U</div>\n    <button\n      type="button"\n      class="sidebar-logo-toggle"\n      [class.sidebar-logo-toggle--collapsed]="collapsed()"\n      (click)="toggle()"\n      [attr.aria-label]="collapsed() ? i18n.nav().sidebarExpand : i18n.nav().sidebarCollapse"\n      [attr.aria-expanded]="!collapsed()"\n      [attr.title]="collapsed() ? i18n.nav().sidebarExpand : i18n.nav().sidebarCollapse"\n    >\n      @if (collapsed()) {\n        <span class="sidebar-logo-toggle__letter" aria-hidden="true">S</span>\n      } @else {\n        <svg\n          class="sidebar-logo-toggle__close"\n          width="20"\n          height="20"\n          viewBox="0 0 24 24"\n          fill="none"\n          stroke="currentColor"\n          stroke-width="2"\n          stroke-linecap="round"\n          aria-hidden="true"\n        >\n          <path d="M18 6 6 18M6 6l12 12"/>\n        </svg>\n      }\n    </button>\n  </div>\n\n  <nav class="nav-links">\n    <a routerLink="/app/home" routerLinkActive="active" class="nav-link">\n      <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">\n        <rect x="3" y="3" width="7" height="7"/><rect x="14" y="3" width="7" height="7"/>\n        <rect x="3" y="14" width="7" height="7"/><rect x="14" y="14" width="7" height="7"/>\n      </svg>\n      <span>{{ i18n.nav().home }}</span>\n    </a>\n    <a routerLink="/app/students" routerLinkActive="active" class="nav-link">\n      <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">\n        <path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/>\n        <circle cx="9" cy="7" r="4"/>\n        <path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/>\n      </svg>\n      <span>{{ i18n.nav().students }}</span>\n    </a>\n    <a routerLink="/app/calendar" routerLinkActive="active" class="nav-link">\n      <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">\n        <rect x="3" y="4" width="18" height="18" rx="2"/>\n        <line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/>\n        <line x1="3" y1="10" x2="21" y2="10"/>\n      </svg>\n      <span>{{ i18n.nav().calendar }}</span>\n    </a>\n    <a routerLink="/app/finance" routerLinkActive="active" class="nav-link">\n      <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">\n        <line x1="12" y1="1" x2="12" y2="23"/>\n        <path d="M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6"/>\n      </svg>\n      <span>{{ i18n.nav().finance }}</span>\n    </a>\n    @if (isSuperAdmin()) {\n      <a routerLink="/app/admin" routerLinkActive="active" class="nav-link">\n        <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">\n          <path d="M12 15v2"/>\n          <path d="M12 3v2"/>\n          <rect x="4" y="8" width="16" height="12" rx="2"/>\n          <path d="M8 8V6a4 4 0 0 1 8 0v2"/>\n        </svg>\n        <span>{{ i18n.nav().admin }}</span>\n      </a>\n    }\n  </nav>\n\n  <div class="sidebar-footer">\n    <a routerLink="/app/account" routerLinkActive="active" class="sidebar-btn sidebar-btn--account">\n      <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">\n        <path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/>\n        <circle cx="12" cy="7" r="4"/>\n      </svg>\n      <span>{{ i18n.nav().account }}</span>\n    </a>\n\n    <button type="button" class="sidebar-btn logout" (click)="logout()">\n      <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">\n        <path d="M9 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h4"/>\n        <polyline points="16 17 21 12 16 7"/><line x1="21" y1="12" x2="9" y2="12"/>\n      </svg>\n      <span>{{ i18n.nav().logout }}</span>\n    </button>\n  </div>\n</aside>\n\n<!-- Mobile bottom nav -->\n<nav class="bottom-nav">\n  <a routerLink="/app/home" routerLinkActive="active" class="bottom-link">\n    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">\n      <rect x="3" y="3" width="7" height="7"/><rect x="14" y="3" width="7" height="7"/>\n      <rect x="3" y="14" width="7" height="7"/><rect x="14" y="14" width="7" height="7"/>\n    </svg>\n    {{ i18n.nav().home }}\n  </a>\n  <a routerLink="/app/students" routerLinkActive="active" class="bottom-link">\n    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">\n      <path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/>\n      <circle cx="9" cy="7" r="4"/>\n    </svg>\n    {{ i18n.nav().students }}\n  </a>\n  <a routerLink="/app/calendar" routerLinkActive="active" class="bottom-link">\n    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">\n      <rect x="3" y="4" width="18" height="18" rx="2"/>\n      <line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/>\n      <line x1="3" y1="10" x2="21" y2="10"/>\n    </svg>\n    {{ i18n.nav().calendar }}\n  </a>\n  <a routerLink="/app/finance" routerLinkActive="active" class="bottom-link">\n    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">\n      <line x1="12" y1="1" x2="12" y2="23"/>\n      <path d="M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6"/>\n    </svg>\n    {{ i18n.nav().finance }}\n  </a>\n  @if (isSuperAdmin()) {\n    <a routerLink="/app/admin" routerLinkActive="active" class="bottom-link">\n      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">\n        <path d="M12 15v2"/>\n        <path d="M12 3v2"/>\n        <rect x="4" y="8" width="16" height="12" rx="2"/>\n        <path d="M8 8V6a4 4 0 0 1 8 0v2"/>\n      </svg>\n      {{ i18n.nav().admin }}\n    </a>\n  }\n  <a routerLink="/app/account" routerLinkActive="active" class="bottom-link">\n    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">\n      <path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/>\n      <circle cx="12" cy="7" r="4"/>\n    </svg>\n    {{ i18n.nav().account }}\n  </a>\n</nav>\n', styles: ["/* src/app/shared/navbar/navbar.component.scss */\n:host {\n  display: contents;\n}\n.sidebar {\n  position: fixed;\n  inset: 0 auto 0 0;\n  width: var(--sidebar-w, 220px);\n  height: 100vh;\n  height: 100dvh;\n  height: 100svh;\n  max-height: 100dvh;\n  max-height: 100svh;\n  background: var(--nav-bg);\n  border-right: 1px solid var(--nav-border);\n  display: flex;\n  flex-direction: column;\n  padding: 16px 8px;\n  padding-bottom: max(16px, env(safe-area-inset-bottom, 0px));\n  transition: width 0.25s ease;\n  overflow: hidden;\n  overscroll-behavior: none;\n  touch-action: manipulation;\n  z-index: var(--z-app-navbar, 100);\n  box-sizing: border-box;\n}\n@media (max-width: 768px), (max-height: 440px) {\n  .sidebar {\n    display: none;\n  }\n}\n.sidebar-head {\n  flex-shrink: 0;\n  padding: 0 4px 12px;\n  min-height: 44px;\n}\n.sidebar-logo {\n  font-size: 17px;\n  font-weight: 600;\n  color: var(--nav-accent);\n  padding: 8px 8px 0;\n  white-space: nowrap;\n  overflow: hidden;\n}\n@media (max-width: 890px) {\n  .sidebar-logo {\n    display: none;\n  }\n}\n.sidebar-logo-toggle {\n  display: none;\n  align-items: center;\n  width: 100%;\n  margin: 0;\n  padding: 10px 12px;\n  border: none;\n  border-radius: 8px;\n  background: transparent;\n  color: var(--nav-text);\n  text-align: left;\n  cursor: pointer;\n  transition: background 0.15s, color 0.15s;\n}\n@media (max-width: 890px) {\n  .sidebar-logo-toggle {\n    display: flex;\n  }\n}\n.sidebar-logo-toggle:hover {\n  background: var(--nav-hover);\n  color: var(--nav-accent);\n}\n.sidebar-logo-toggle--collapsed {\n  justify-content: center;\n  color: var(--nav-accent);\n}\n.sidebar-logo-toggle__letter {\n  font-size: 17px;\n  font-weight: 700;\n  line-height: 1;\n}\n.sidebar-logo-toggle__close {\n  flex-shrink: 0;\n  opacity: 0.85;\n}\n.nav-links {\n  display: flex;\n  flex-direction: column;\n  gap: 4px;\n  flex: 1 1 auto;\n  min-height: 0;\n  overflow: hidden;\n  overscroll-behavior: none;\n  touch-action: none;\n}\n.nav-link {\n  display: flex;\n  align-items: center;\n  gap: 12px;\n  padding: 10px 12px;\n  border-radius: 8px;\n  color: var(--nav-text);\n  text-decoration: none;\n  font-size: 14px;\n  white-space: nowrap;\n  touch-action: manipulation;\n  transition: background 0.15s, color 0.15s;\n}\n.nav-link svg {\n  flex-shrink: 0;\n}\n.nav-link:hover {\n  background: var(--nav-hover);\n}\n.nav-link.active {\n  background: var(--nav-active-bg);\n  color: var(--nav-accent);\n  font-weight: 500;\n}\n.nav-link span {\n  transition: opacity 0.2s ease;\n}\n.sidebar.collapsed .nav-link span {\n  opacity: 0;\n  width: 0;\n  overflow: hidden;\n}\n.sidebar-btn {\n  background: none;\n  border: none;\n  cursor: pointer;\n  padding: 10px 12px;\n  border-radius: 8px;\n  color: var(--nav-text);\n  display: flex;\n  align-items: center;\n  gap: 12px;\n  font-size: 14px;\n  white-space: nowrap;\n  width: 100%;\n  touch-action: manipulation;\n  transition: background 0.15s, color 0.15s;\n}\n.sidebar-btn svg {\n  flex-shrink: 0;\n}\n.sidebar-btn span {\n  transition: opacity 0.2s ease;\n}\n.sidebar.collapsed .sidebar-btn span {\n  opacity: 0;\n  width: 0;\n  overflow: hidden;\n}\n.sidebar-btn:hover {\n  background: var(--nav-hover);\n}\n@media (max-width: 768px), (max-height: 440px) {\n  .sidebar-btn.logout {\n    display: none;\n  }\n}\n.sidebar-btn.logout:hover {\n  background: #fce8e6;\n  color: #c5221f;\n}\n.sidebar-footer {\n  display: flex;\n  flex-shrink: 0;\n  flex-direction: column;\n  gap: 4px;\n  margin-top: auto;\n  padding-top: 10px;\n  border-top: 1px solid var(--nav-border);\n  box-sizing: border-box;\n}\n.sidebar-btn--account {\n  text-decoration: none;\n}\n.sidebar-btn--account.active {\n  background: var(--nav-active-bg);\n  color: var(--nav-accent);\n  font-weight: 500;\n}\n.sidebar.collapsed .nav-link,\n.sidebar.collapsed .sidebar-btn {\n  justify-content: center;\n  gap: 0;\n  padding-left: 10px;\n  padding-right: 10px;\n}\n.sidebar.collapsed .sidebar-btn span {\n  opacity: 0;\n  width: 0;\n  overflow: hidden;\n}\n.bottom-nav {\n  display: none;\n}\n@media (max-width: 768px), (max-height: 440px) {\n  .bottom-nav {\n    display: flex;\n    position: fixed;\n    bottom: 0;\n    left: 0;\n    right: 0;\n    height: calc(56px + env(safe-area-inset-bottom, 0px));\n    min-height: 56px;\n    padding-bottom: env(safe-area-inset-bottom, 0px);\n    background: var(--nav-bg);\n    border-top: 1px solid var(--nav-border);\n    justify-content: space-around;\n    align-items: center;\n    overflow: hidden;\n    overscroll-behavior: none;\n    touch-action: none;\n    z-index: var(--z-app-navbar);\n  }\n}\n.bottom-link {\n  display: flex;\n  min-width: 0;\n  flex-direction: column;\n  align-items: center;\n  gap: 3px;\n  color: var(--nav-text);\n  text-decoration: none;\n  font-size: 10px;\n  flex: 1;\n  padding: 6px 0;\n  overflow: hidden;\n  text-align: center;\n  line-height: 1.15;\n  transition: color 0.15s;\n}\n.bottom-link svg {\n  width: 22px;\n  height: 22px;\n  flex-shrink: 0;\n}\n.bottom-link.active {\n  color: var(--nav-accent);\n}\n/*# sourceMappingURL=navbar.component.css.map */\n"] }]
  }], () => [], null);
})();
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && \u0275setClassDebugInfo(NavbarComponent, { className: "NavbarComponent", filePath: "app/shared/navbar/navbar.component.ts", lineNumber: 16 });
})();

// src/app/core/services/analytics.service.ts
var AnalyticsService2 = class _AnalyticsService {
  platformId = inject(PLATFORM_ID);
  ...
  constructor() {
    ...
    this.router.events.pipe(filter((event) => event instanceof NavigationEnd), takeUntilDestroyed(this.destroyRef)).subscribe((event) => this.trackPageView(event.urlAfterRedirects));
    effect(() => {
  ...
  trackEvent(eventName, params) {
    ...
    logEvent2(this.analytics, eventName, params);
  }
  isEnabled() {
    return environment.production && isPlatformBrowser(this.platformId) && Boolean(this.analytics) && Boolean(environment.firebaseConfig.measurementId);
  }
  canTrack() {
  ...
  applyConsent(enabled) {
    ...
    setAnalyticsCollectionEnabled2(this.analytics, enabled);
    setConsent2({
  ...
  trackPageView(path) {
    ...
    const pageLocation = typeof window !== "undefined" ? `${window.location.origin}${path}` : path;
    logEvent2(this.analytics, "page_view", {
      page_path: path,
...
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && setClassMetadata(AnalyticsService2, [{
    type: Injectable,
...
var PAGE_TITLE_KEYS = /* @__PURE__ */ new Set([
  "default",
  "landing",
  "login",
  "register",
...
var SeoService = class _SeoService {
  ...
  currentKey = null;
  constructor() {
    this.router.events.pipe(filter((event) => event instanceof NavigationEnd), takeUntilDestroyed(this.destroyRef)).subscribe(() => this.updateFromActiveRoute());
    effect(() => {
      this.i18n.lang();
      this.refreshTitle();
    });
  ...
  resolveRouteTitleKey() {
    ...
    return found && PAGE_TITLE_KEYS.has(found) ? found : null;
  }
...
var App = class _App {
  ...
  static \u0275cmp = /* @__PURE__ */ \u0275\u0275defineComponent({ type: _App, selectors: [["app-root"]], decls: 2, vars: 1, consts: [[1, "app-shell"], [1, "app-content"], [1, "app-page-host"]], template: function App_Template(rf, ctx) {
    if (rf & 1) {
      \u0275\u0275conditionalCreate(0, App_Conditional_0_Template, 5, 0, "div", 0)(1, App_Conditional_1_Template, 1, 0, "router-outlet");
    }
    if (rf & 2) {
      \u0275\u0275conditional(ctx.showNavbar() ? 0 : 1);
    }
...
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && setClassMetadata(App, [{
    type: Component,
    args: [{ selector: "app-root", imports: [RouterOutlet, NavbarComponent], template: '@if (showNavbar()) {\n  <div class="app-shell">\n    <app-navbar />\n    <div class="app-content">\n      <div class="app-page-host">\n        <router-outlet />\n      </div>\n    </div>\n  </div>\n} @else {\n  <router-outlet />\n}\n' }]
  }], () => [], null);
})();
(() => {
