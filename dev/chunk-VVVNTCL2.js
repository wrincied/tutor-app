...
function resolveRegisterError(error, strings) {
  const methods = error instanceof EmailAlreadyRegisteredError ? error.signInMethods : [];
...
function resolveLoginError(error, strings) {
...
// src/app/core/utils/post-auth-navigation.ts
function postAuthPath(profile, emailVerified) {
  if (!emailVerified) {
...
var AuthService = class _AuthService {
   ...
   */
  redirectResult$;
  firebaseUser = toSignal(authState2(this.auth), { initialValue: null });
  isLoggedIn = computed(() => this.firebaseUser() !== null, ...ngDevMode ? [{ debugName: "isLoggedIn" }] : (
    /* istanbul ignore next */
    []
  ));
  emailVerified = computed(() => this.firebaseUser()?.emailVerified === true, ...ngDevMode ? [{ debugName: "emailVerified" }] : (
    /* istanbul ignore next */
    []
  ));
  /** Вход по email/паролю — можно менять пароль в аккаунте. Только Google — нет. */
  canChangePassword = computed(() => this.hasPasswordProvider(this.firebaseUser()), ...ngDevMode ? [{ debugName: "canChangePassword" }] : (
    /* istanbul ignore next */
    []
  ));
  hasPasswordProvider(user3) {
    return Boolean(user3?.providerData.some((provider) => provider.providerId === EmailAuthProvider.PROVIDER_ID));
  }
  appBaseUrl() {
    return environment.appUrl?.replace(/\/$/, "") || (typeof window !== "undefined" ? window.location.origin : "http://localhost:4200");
  }
  ...
  sendPasswordReset(email) {
    ...
    return this.fromAuth(() => sendPasswordResetEmail2(this.auth, normalized, this.passwordResetActionCodeSettings()));
  }
  register(email, password) {
    const normalized = email.trim().toLowerCase();
    return this.fromAuth(() => fetchSignInMethodsForEmail2(this.auth, normalized)).pipe(switchMap((methods) => {
      if (methods.length > 0) {
        return throwError(() => new EmailAlreadyRegisteredError(methods));
      }
      return this.fromAuth(() => createUserWithEmailAndPassword2(this.auth, normalized, password));
    }), switchMap((cred) => this.fromAuth(() => sendEmailVerification2(cred.user, this.verificationActionCodeSettings())).pipe(map(() => cred.user))), switchMap((user3) => this.bootstrapProfile().pipe(map(() => user3))));
  }
  login(email, password) {
    const normalized = email.trim().toLowerCase();
    return this.fromAuth(() => signInWithEmailAndPassword2(this.auth, normalized, password)).pipe(switchMap((cred) => this.afterFirebaseSignIn(cred.user)), catchError((err) => this.enrichLoginError(err, normalized)));
  }
  enrichLoginError(err, email) {
    ...
    return this.fromAuth(() => fetchSignInMethodsForEmail2(this.auth, email)).pipe(switchMap((methods) => {
      const hasGoogle = methods.includes(GoogleAuthProvider.PROVIDER_ID);
      const hasPassword = methods.includes(EmailAuthProvider.PROVIDER_ID);
      if (hasGoogle && !hasPassword) {
  ...
  loginWithGooglePopup() {
    const provider = new GoogleAuthProvider();
    provider.setCustomParameters({ prompt: "select_account" });
    return this.fromAuth(() => signInWithPopup2(this.auth, provider)).pipe(switchMap((cred) => this.afterFirebaseSignIn(cred.user)));
  }
  /**
  ...
  navigateAfterAuth(profile, user3) {
    const path = postAuthPath(profile, user3.emailVerified === true);
    const queryParams = profile.data_consent_accepted === false ? { consent: "declined" } : void 0;
    void this.router.navigate([path], { queryParams });
  }
  /** Вызываем bootstrap для создания/синхронизации профиля на бэкенде Node.js */
  afterFirebaseSignIn(user3) {
    return this.bootstrapProfile().pipe(map(() => user3));
  }
  resendVerificationEmail() {
    ...
    return this.fromAuth(() => sendEmailVerification2(user3, this.verificationActionCodeSettings()));
  }
  reloadUser() {
    const user3 = this.auth.currentUser;
    if (!user3) {
    ...
    return this.fromAuth(() => user3.reload()).pipe(map(() => this.auth.currentUser));
  }
  bootstrapProfile() {
    return this.http.post(`${API}/auth/bootstrap`, {});
  }
  async getIdToken() {
    const user3 = this.auth.currentUser;
    if (!user3) {
    ...
    return user3.getIdToken();
  }
  updateEmailWithGoogleReauth(newEmail) {
    const user3 = this.auth.currentUser;
    if (!user3) {
      throw new Error("Not signed in");
    }
    ...
    return this.fromAuth(() => reauthenticateWithPopup2(user3, provider)).pipe(switchMap(() => this.fromAuth(() => updateEmail2(user3, normalized))), switchMap(() => this.fromAuth(() => sendEmailVerification2(user3, this.verificationActionCodeSettings()))), switchMap(() => this.fromAuth(() => user3.reload())), map(() => this.auth.currentUser));
  }
  updateCredentials(options) {
    ...
    const credential = EmailAuthProvider.credential(user3.email, options.currentPassword);
    return this.fromAuth(() => reauthenticateWithCredential2(user3, credential)).pipe(switchMap(() => {
      const tasks = [];
      if (options.newPassword) {
        tasks.push(updatePassword2(user3, options.newPassword));
      }
      if (options.newEmail) {
        tasks.push(updateEmail2(user3, options.newEmail.trim().toLowerCase()));
      }
      return tasks.length ? this.fromAuth(() => Promise.all(tasks)) : from(Promise.resolve());
    }), switchMap(() => {
      if (options.newEmail) {
        return this.fromAuth(() => sendEmailVerification2(user3, this.verificationActionCodeSettings())).pipe(switchMap(() => this.fromAuth(() => user3.reload())));
      }
      return this.fromAuth(() => user3.reload());
    }), map(() => this.auth.currentUser));
  }
  logout() {
    return this.fromAuth(() => signOut2(this.auth)).pipe(map(() => {
